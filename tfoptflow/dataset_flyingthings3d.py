...
class FlyingThings3DDataset(OpticalFlowDataset):
    ...
    def __init__(self, mode='train_with_val', ds_root=_FLYINGTHINGS3D_ROOT, options=_DEFAULT_DS_TRAIN_OPTIONS):
        ...
        """
        self.min_flow_mag = 0.
        self.avg_flow_mag = 37.99934768676758
        self.max_flow_mag = 1714.8948974609375
    ...
    def set_folders(self):
        ...
        Sample results:
            self._train_folder           = 'E:/datasets/FlyingThings3D/frames_cleanpass/TRAIN'
            self._train_label_folder     = 'E:/datasets/FlyingThings3D/optical_flow/TRAIN'
            self._val_folder             = 'E:/datasets/FlyingThings3D/frames_cleanpass/TRAIN'
            self._val_label_folder       = 'E:/datasets/FlyingThings3D/optical_flow/TRAIN'
            self._val_pred_label_folder  = 'E:/datasets/FlyingThings3D/flow_pred'
            self._test_folder            = 'E:/datasets/FlyingThings3D/frames_cleanpass/TEST'
            self._test_pred_label_folder = 'E:/datasets/FlyingThings3D/flow_pred'
        """
        self._train_folder = f"{self._ds_root}/frames_cleanpass/TRAIN"
        self._val_folder = self._train_folder
        self._test_folder = f"{self._ds_root}/frames_cleanpass/TEST"

        self._train_label_folder = f"{self._ds_root}/optical_flow/TRAIN"
    ...
    def _build_ID_sets(self):
        ...
        self._IDs = []
        for subset in sorted(os.listdir(self._train_folder)): # subset: 'A'
            scenes = sorted(os.listdir(f"{self._train_folder}/{subset}"))
            for scene in scenes: # scene: '0000'
                frames = sorted(os.listdir(f"{self._train_folder}/{subset}/{scene}/left"))
        ...
        self._test_IDs = []
        for subset in sorted(os.listdir(self._test_folder)): # subset: 'A'
            scenes = sorted(os.listdir(f"{self._test_folder}/{subset}"))
            for scene in scenes: # scene: '0000'
                frames = sorted(os.listdir(f"{self._test_folder}/{subset}/{scene}/left"))
                for idx in range(len(frames)-1):
                    frame1_ID = f'{subset}/{scene}/left/{frames[idx]}'
                    frame2_ID = f'{subset}/{scene}/left/{frames[idx+1]}'
                    frame = frames[idx].split('.')[0]
...
class FlyingThings3DHalfResDataset(FlyingThings3DDataset):
    ...
    def __init__(self, mode='train_with_val', ds_root=_FLYINGTHINGS3DHALFRES_ROOTS, options=_DEFAULT_DS_TRAIN_OPTIONS):
        ...
        if type(ds_root) is list or type(ds_root) is tuple:
            self._ds_root_fullres, self._ds_root_halfres = ds_root
            if os.path.exists(self._ds_root_halfres):
                self.generate_files = False
                ds_root = self._ds_root_halfres
            else:
                self.generate_files = True
                ds_root = self._ds_root_halfres
                # ds_root = self._ds_root_fullres
        super().__init__(mode, ds_root, options)
        self.min_flow_mag = 0.
        self.avg_flow_mag = 18.954660415649414
        self.max_flow_mag = 856.9017944335938
        if 'type' not in self.opts:
            self.opts['type'] = 'into_future'
    ...
    def set_folders(self):
        ...
        Overriden by each dataset. Called by the base class on init.
        Sample results:
            self._train_folder           = 'E:/datasets/FlyingThings3D_HalfRes/frames_cleanpass/TRAIN'
            self._train_label_folder     = 'E:/datasets/FlyingThings3D_HalfRes/optical_flow/TRAIN'
            self._val_folder             = 'E:/datasets/FlyingThings3D_HalfRes/frames_cleanpass/TRAIN'
            self._val_label_folder       = 'E:/datasets/FlyingThings3D_HalfRes/optical_flow/TRAIN'
            self._val_pred_label_folder  = 'E:/datasets/FlyingThings3D_HalfRes/flow_pred'
            self._test_folder            = 'E:/datasets/FlyingThings3D_HalfRes/frames_cleanpass/TEST'
            self._test_pred_label_folder = 'E:/datasets/FlyingThings3D_HalfRes/flow_pred'
        """
        self._train_folder = f"{self._ds_root}/frames_cleanpass/TRAIN"
        self._val_folder = self._train_folder
        self._test_folder = f"{self._ds_root}/frames_cleanpass/TEST"
    ...
    def _build_ID_sets(self):
        ...
        if self.generate_files is True:
            ds_root = self._ds_root_fullres
            train_folder = self._train_folder.replace(self._ds_root_halfres, self._ds_root_fullres)
            train_label_folder = self._train_label_folder.replace(self._ds_root_halfres, self._ds_root_fullres)
            test_folder = self._test_folder.replace(self._ds_root_halfres, self._ds_root_fullres)
        else:
            ds_root = self._ds_root_halfres
            train_folder = self._train_folder
            train_label_folder = self._train_label_folder
        ...
        # Search the train folder for the samples, create string IDs for them
        self._IDs = []
        for subset in sorted(os.listdir(train_folder)): # subset: 'A'
            scenes = sorted(os.listdir(f"{train_folder}/{subset}"))
            for scene in scenes: # scene: '0000'
                frames = sorted(os.listdir(f"{train_folder}/{subset}/{scene}/left"))
                for idx in range(len(frames)-1):
                    frame1_ID = f'{subset}/{scene}/left/{frames[idx]}'
                    frame2_ID = f'{subset}/{scene}/left/{frames[idx+1]}'
                    frame = frames[idx].split('.')[0]
        ...
        if self.generate_files is True:
            if not os.path.exists(self._ds_root_halfres):
                os.makedirs(self._ds_root_halfres)
            with tqdm(total=len(self._IDs), desc="Downsampling train images & flows", ascii=True, ncols=100) as pbar:
                for n, ID in enumerate(self._IDs):
                    pbar.update(1)
                    flow_path_fullres = f'{train_label_folder}/{ID[2]}'
                    flow_path_halfres = flow_path_fullres.replace(self._ds_root_fullres, self._ds_root_halfres).replace('.pfm', '.flo')
                    if not os.path.exists(flow_path_halfres):
                        flow = flow_read(flow_path_fullres)
                        flow = cv2.resize(flow, (int(flow.shape[1] * 0.5), int(flow.shape[0] * 0.5))) * 0.5
                        flow_write(flow, flow_path_halfres)
                    for frame_ID in [ID[0], ID[1]]:
                        frame_path_fullres = f'{train_folder}/{frame_ID}'
                        frame_path_halfres = frame_path_fullres.replace(self._ds_root_fullres, self._ds_root_halfres)
        ...
        # Build the test dataset
        self._test_IDs = []
        for subset in sorted(os.listdir(test_folder)): # subset: 'A'
            scenes = sorted(os.listdir(f"{test_folder}/{subset}"))
            for scene in scenes: # scene: '0000'
                frames = sorted(os.listdir(f"{test_folder}/{subset}/{scene}/left"))
                for idx in range(len(frames)-1):
                    frame1_ID = f'{subset}/{scene}/left/{frames[idx]}'
                    frame2_ID = f'{subset}/{scene}/left/{frames[idx+1]}'
                    frame = frames[idx].split('.')[0]
        ...
        if self.generate_files is True:
            with tqdm(total=len(self._test_IDs), desc="Downsampling test images", ascii=True, ncols=100) as pbar:
                for ID in self._test_IDs:
                    pbar.update(1)
                    for frame_ID in [ID[0], ID[1]]:
                        frame_path_fullres = f'{test_folder}/{frame_ID}'
                        frame_path_halfres = frame_path_fullres.replace(self._ds_root_fullres, self._ds_root_halfres)
                        if not os.path.exists(frame_path_halfres):
                            frame = imread(frame_path_fullres)
                            frame = cv2.resize(frame, (int(frame.shape[1] * 0.5), int(frame.shape[0] * 0.5)))
